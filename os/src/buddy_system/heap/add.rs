...
impl<const ORDER: usize> super::Heap<ORDER> {
    ...
    pub unsafe fn add_to_heap(&mut self, mut start: usize, mut end: usize) {
        ...
        while current_start + size_of::<usize>() <= end {
            // 计算当前可分配块的大小及阶次
            let (size, order) = self.calc_block_size_and_order(current_start, end - current_start);
            // 累加统计
            total += size;
    ...
    fn calc_block_size_and_order(&self, addr: usize, remaining: usize) -> (usize, usize) {
        // 基于 addr 的 lowbit 和剩余空间最大幂次确定初始块大小
        let lowbit = addr & (!addr + 1);
        let mut size = core::cmp::min(lowbit, super::prev_power_of_two(remaining));
        // 阶次由块大小的 trailing zeros 得到
